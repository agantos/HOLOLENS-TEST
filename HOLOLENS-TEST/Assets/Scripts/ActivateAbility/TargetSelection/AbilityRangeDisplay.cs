...
public class AbilityRangeDisplay : MonoBehaviour
{
    ...
    void SetPosition()
    {
        Vector3 attackerPosition = GameManager.GetInstance().playingCharacterGameObjects[CastingAbilityManager.attacker.name].transform.localPosition;
        float bottomOfAttacker = attackerPosition.y - GameManager.GetInstance().playingCharacterGameObjects[CastingAbilityManager.attacker.name].transform.localScale.y;
